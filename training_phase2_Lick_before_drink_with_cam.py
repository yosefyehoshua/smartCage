...
def StartStimulus(trialType):
    ...
    if stimulusType == 'pole':
        if trialType == 'Go':
            wiringpi.pwmWrite(PIN_SERVO, Go_Angle)
        elif trialType == 'NoGo':
            wiringpi.pwmWrite(PIN_SERVO, NoGo_Angle)
...
def StopStimulus():
    ...
    if stimulusType == 'pole':
        wiringpi.pwmWrite(PIN_SERVO, SteadyState_Angle)     # steady state angle


# run a trial of a task
def runTrial_task(trialType,mouseRFID,stimulusOff,giveReward):
    StartStimulus(stimulusType,trialType)   # start stimulus
    ...
    while time.time() < t_end_response:                                
        if time.time() >= t_end_stimulus and stimulusOff == 0:
            StopStimulus(stimulusType)  # stop stimulus
            stimulusOff = 1
...
USB_PORT_user = "/dev/ttyUSB0"
PIN_WATER_user = 25   
PIN_LICK_DETECTOR_user = 2   # pin in the lick detector that is connected to the water spout

# choose stimulus type: light or pole
stimulusType_user = 'whisker'
...
# camera params
global imagesDir
imagesDir = '/home/pi/smartcageData/images2018/'
# data params
...
PIN_LED = 20
global PIN_SERVO
PIN_SERVO = 18  # don't change, a unique pin for PWM (pulse width modulation)
# time params in sec
global WATER_PULSE_DURATION
WATER_PULSE_DURATION = 0.035  #0.072
